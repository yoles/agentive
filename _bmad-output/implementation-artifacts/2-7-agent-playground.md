...
3. **Isolation via PAS d'import** — la garantie AC2 vient de la structure : `PlaygroundService` n'importe PAS `shared.event_bus.publish` directement (sauf l'unique helper d'audit AC5 single-line), n'importe PAS `MemoryChunkRepo`, n'importe PAS `instance_repo`. Cette discipline est vérifiable via `import-linter` Contract 1 (`features-isolated`, cf Story 1.5) — le module `agentive_backend.features.m7_playground` est enregistré comme co-tenant du slot M7 aux côtés de `m7_chat`. Le slot M7 héberge donc deux features : `m7_chat` (runtime user feature, FR47) et `m7_playground` (dev/QA tool, FR48). Cette co-tenance est intentionnelle et résout B-02/B-04 de la code-review Story 2.7 ; la garantie d'isolation reste identique car Contract 1 interdit toute import croisé entre features listées (donc `m7_playground ⇎ m7_chat` également).
