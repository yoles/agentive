...
async def test_instantiate_template_happy_path_creates_instance_with_snapshot(
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        # Audit event agent_registry.agent_instance.created published.
        async with seed_session_factory() as session:
...
async def test_instantiate_template_isolation_v2_v3_no_hot_swap(
    ...
    * P-06 : asserte que 2 events `agent_registry.agent_instance.created` sont publiés
...
async def test_instantiate_template_atomicity_publish_failure_rolls_back_insert(
    ...
    ``agent_registry.agent_instance.created`` (le ``_create_template`` setup utilise
    aussi ``service.publish`` pour ``agent_registry.agent_template.created``, on ne
    ...
    async def _selective_explode(event_type: str, *args: Any, **kwargs: Any) -> Any:
        if event_type == "agent_registry.agent_instance.created":
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        # `agent_registry.agent_template.created` which must succeed so the template row
