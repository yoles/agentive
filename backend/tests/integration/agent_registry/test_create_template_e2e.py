...
async def test_create_template_happy_path_persists_row_and_publishes_event(
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        count = await _count_outbox(
            seed_session_factory, "agent_registry.agent_template.created", name="Code Producer"
        )
        assert count == 1
...
async def test_create_template_unknown_archetype_returns_422_rfc7807(
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        count = await _count_outbox(
            seed_session_factory, "agent_registry.agent_template.created", name="Test"
        )
        assert count == 0
...
async def test_create_template_empty_name_returns_422_rfc7807(
    ...
    async with seed_session_factory() as session:
        ...
        assert int(result.scalar_one()) == 0
    count = await _count_outbox(
        seed_session_factory, "agent_registry.agent_template.created", name=""
    )
    assert count == 0
...
async def test_create_template_whitespace_only_name_returns_422_rfc7807(
    ...
    async with seed_session_factory() as session:
        ...
        assert int(result.scalar_one()) == 0
    count_blank = await _count_outbox(
        seed_session_factory, "agent_registry.agent_template.created", name=""
    )
...
async def test_create_template_propagates_correlation_id(
    ...
    event = await _fetch_outbox_payload(
        seed_session_factory, "agent_registry.agent_template.created", name="Curious George"
    )
...
async def test_create_template_empty_name_returns_422_without_writes(
    ...
    assert (
        await _count_outbox(
            ...
            "agent_registry.agent_template.created",
            name="",
        )
        == 0
    )
...
async def test_publish_failure_rolls_back_template_and_outbox(
    ...
    service_module = importlib.import_module("agentive_backend.features.agent_registry.service")
