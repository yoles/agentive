        ...
        async with seed_session_factory() as session:
            assigned = await session.execute(
                text(
                    ...
                    "WHERE event_type = 'agent_registry.agent_template.tool_assigned' "
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        async with seed_session_factory() as session:
            ...
            unassigned_count = await session.execute(
                text(
                    "SELECT COUNT(*) FROM outbox_events "
                    "WHERE event_type = 'agent_registry.agent_template.tool_unassigned' "
                    "AND payload->>'template_id' = :tid"
                ),
                {"tid": template_id},
            )
...
async def test_delete_template_tool_happy_path(
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        # agent_registry.agent_template.tool_unassigned émis". Verify the outbox row
...
async def test_replace_assign_atomicity_publish_failure_rolls_back(
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        async def _selective_explode(event_type: str, *args: Any, **kwargs: Any) -> Any:
            if event_type == "agent_registry.agent_template.tool_assigned":
        ...
        async with seed_session_factory() as session:
            spurious = await session.execute(
                text(
                    "SELECT COUNT(*) FROM outbox_events "
                    "WHERE event_type IN ('agent_registry.agent_template.tool_assigned', "
                    "                      'agent_registry.agent_template.tool_unassigned') "
                    "AND payload->>'template_id' = :tid"
                ),
                {"tid": template_id},
            )
