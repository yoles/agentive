...
Pattern miroir de :mod:`tests.integration.m2_agent_registry.test_create_template_e2e` —
...
from tests.integration.m2_agent_registry.test_create_template_e2e import (
    _auth_headers,
    _count_outbox,
    _make_app,
)
...
async def test_put_template_with_system_prompt_bumps_version_and_inserts_prompt(
    ...
    count = await _count_outbox(seed_session_factory, "m2.agent_template.updated")
    assert count >= 1
