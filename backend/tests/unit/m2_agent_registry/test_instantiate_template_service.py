...
``tests/integration/m2_agent_registry/test_instantiate_template_e2e.py``.
...
def _make_service(
    *,
    ...
    """Build a service with mocked repos.
...
async def test_instantiate_template_happy_path_captures_snapshot_and_publishes_event(
    ...
    """AC1 — snapshot complet figé + event m2.agent_instance.created publié."""
    template = SimpleNamespace(
        id=uuid4(),
        name="Code Producer",
        archetype="producteur",
    ...
    assert event_type == "m2.agent_instance.created"
