...
- [`run-node-output-exposure.md`](./run-node-output-exposure.md) — **Story 5.7** : où vit la sortie complète d'un node et par où elle sort. Elle est **lue à la demande dans le checkpointer LangGraph**, jamais recopiée — élargir l'aperçu ferait payer sa taille à `workflow_runs.checkpoint`, réécrit à chaque superstep et relu à chaque reconnexion SSE ; la persister ailleurs créerait une copie que **rien ne purge**, alors que la rétention de la Story 4.10 couvre déjà ces tables. Deux routes dédiées, la frame SSE inchangée. Contrat de troncature en trois lignes, et **deux champs orthogonaux** (`node_outputs_source` / `checkpointer_reachable`) parce qu'« d'où ça vient » et « ai-je pu consulter » sont deux faits — les confondre faisait lire une panne de pool comme une purge définitive.
- [`dev-pole-agent-handoff-contract.md`](./dev-pole-agent-handoff-contract.md) — **Story 5.3** : deux arbitrages sur ce qu'un agent du Pôle Dev voit avant de produire. (1) **Régime de passage** — `UpdateTemplateRequest` porte désormais `include_raw_previous_output`, parce que le levier existait dans le moteur depuis la Story 4.7 sans aucun chemin déclaratif : sur quatre nodes, un contrat structuré traversait trois condensations, et les `approach.steps[].id` que le Code Producer doit citer ne survivent pas à un résumé. L'opt-out se paie en volume amont (`MAX_UPSTREAM_OUTPUT_CHARS` devient la seule borne) et se rembourse en appels : deux arêtes ne produisent plus de résumé du tout, donc 5 appels LLM par run au lieu de 7. (2) **Conventions du projet** — le namespace `métier` n'injecte rien au runtime, rien n'écrit dans `dev-metier`, et `CONVENTIONS.md` est hors de portée des outils (le container ne monte que `./backend:/app`, qui ne contient aucun `.md`) : elles sont portées par le `system_prompt`, le namespace est déclaré pour ce qu'il fait réellement, et élargir `AGENTIVE_DEV_CODE_ROOTS` est refusé plutôt que laissé ouvert.
