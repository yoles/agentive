...
- La plage utile de `AGENTIVE_TOOL_LOOP_MAX_WALL_CLOCK_S` est **120–200 s**, et les deux bornes sont calculées, pas choisies : en dessous de 120 s on tuerait un nœud sans aucun outil qui bascule d'un provider à l'autre (la valeur est refusée au démarrage) ; au-dessus de ~202,5 s la fenêtre de détection d'un run planté dépasse une heure à la pire combinaison légale des autres réglages. Chaque seconde de budget d'outils en plus coûte dix secondes de fenêtre de détection.
